...
main(int argc, char** argv) {
    ...
    if(debug == 1) printf("♥♥♥ pid = %d esta com array completo, seus limites sao ini= %d e fim= %d, mas pode ver todo, exemplo? 5001 = %d,\n", my_rank, ini_vetor, fim_vetor, vetor[5001]);
    ...
    while (pronto == 0) {
        ...
        if((status.MPI_TAG == tag_inicio) || (status.MPI_TAG == tag_erro) || (flag_erro == 1) || primeira_vez == 0){
            ...
            if(((negatv + positv) == np-1)){ // Se todas as mensagens CHEGARAM !
                printf("♥♥♥ DEBUG p= %d recebeu todas mensagens  \n", ini_vetor);
